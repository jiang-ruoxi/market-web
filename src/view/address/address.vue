                ...
                <el-table-column align="left" label="操作" min-width="120">
                    <template #default="scope">
                        ...
                        <el-button type="primary" link icon="edit" class="table-button"
                                   @click="updateAddressFunc(scope.row)">变更
                        </el-button>
        ...
        <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="type==='create'?'添加':'修改'"
            ...
            <el-scrollbar height="200px">
                <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
                    <el-form-item label="省份" prop="parent_id">
                        <el-select v-model="formData.parent_id" placeholder="请选择" style="width:100%" :clearable="true">
                            <el-option v-for="item in address_list" :key="item.ID" :label="item.name" :value="item.ID">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="城市:" prop="name">
                        <el-input v-model="formData.name" :clearable="true" placeholder="请输入城市"/>
                    </el-form-item>
...
<script setup>
    ...
    const getTableData = async () => {
        const table = await getAddressList({page: page.value, pageSize: pageSize.value, ...searchInfo.value})
        if (table.code === 0) {
    ...
    const updateAddressFunc = async (row) => {
        ...
        type.value = 'update'
        if (res.code === 0) {
            formData.value = res.data.rezmAddress
    ...
    const getAddressAllListData = async () => {
        const res = await getAddressAllList();
        if (res.code === 0) {
            address_list.value = res.data.list;
        }
    };
    getAddressAllListData()
