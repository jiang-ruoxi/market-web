  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" min-width="120">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateZmTagsFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getZmTagsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteZmTagsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateZmTagsFunc = async(row) => {
    const res = await findZmTags({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rezmTags
...
// 删除行
const deleteZmTagsFunc = async (row) => {
    const res = await deleteZmTags({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createZmTags(formData.value)
                  break
                case 'update':
                  res = await updateZmTags(formData.value)
                  break
                default:
                  res = await createZmTags(formData.value)
                  break
