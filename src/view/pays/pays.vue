  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" min-width="120">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updatePaysFunc(scope.row)">变更</el-button>
...
const onDelete = async() => {
      ...
      const res = await deletePaysByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 删除行
const deletePaysFunc = async (row) => {
    const res = await deletePays({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
